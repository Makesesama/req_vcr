defmodule Reqord.ConcurrentAllModesTest do
  ...
  test ":new_episodes mode works with concurrent requests (no accumulation needed)", %{
    ...
    FileSystem.write_entry(cassette_path, CassetteEntry.to_map(initial_entry))

    # Verify initial state
    initial_entries = CassetteReader.load_entries(cassette_path)
    ...
    tasks = [
      Task.async(fn ->
        ...
        FileSystem.write_entry(cassette_path, CassetteEntry.to_map(entry))
        :ok
      end),
      Task.async(fn ->
    ...
    final_entries = CassetteReader.load_entries(cassette_path)
  ...
  test ":all mode works with concurrent requests (our GenServer fix)", %{test_dir: test_dir} do
    ...
    write_all_entries_for_all_mode(cassette_path, final_entries)
